...
namespace CashmereDeposit.Utils.AlertClasses
{
    public class AlertTransactionCancelled : AlertBase
    {
          ...
          : base(device, dateDetected)
        {
            ...
            AlertType = _depositorDBContext.AlertMessageTypes.FirstOrDefault(x => x.Id == 4005) ?? throw new NullReferenceException(string.Format("{0}:AlertID {1} is not in DB", nameof(AlertTransactionCancelled), 4005));
            if (transaction == null)
