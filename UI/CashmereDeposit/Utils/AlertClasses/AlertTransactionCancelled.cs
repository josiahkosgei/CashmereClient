...
// Type: CashmereDeposit.Utils.AlertClasses.AlertTransactionCancelled
...
namespace CashmereDeposit.Utils.AlertClasses
{
    public class AlertTransactionCancelled : AlertBase
    {
          ...
          : base(device, dateDetected)
        {
            ...
            AlertType = _alertMessageTypeRepository.GetByIdAsync(ALERT_ID).ContinueWith(x => x.Result).Result ?? throw new NullReferenceException(string.Format("{0}:AlertID {1} is not in DB", nameof(AlertTransactionCancelled), 4005));
            if (transaction == null)
