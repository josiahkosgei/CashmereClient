...
namespace CashAccSysDeviceManager
{
    public class CDCMessenger
    {
        ...
        private SortedList<int, string> OutGoingMessageQueue { get; set; } = new();
        ...
        {
            Log = new CashmereLogger(
                Assembly.GetAssembly(typeof(CashAccSysDeviceManager)).GetName().Version.ToString(),
                "DeviceMessengerLog", null);
            Log.Debug(GetType().Name, "Constructor", "Initialisation", "Creating CDCMessenger",
                Array.Empty<object>());
            _host = host;
            _port = port;
            ...
            client.XMLDataReceived += Client_DataReceived;
        ...
        {
            Log.Info(GetType().Name, "Client_ConnectionStatusChanged", "EventHandler",
                "Reset _controllerSequenceNumber on connection established", Array.Empty<object>());
            ...
            try
            {
                ...
                if (message != null)
                {
                    ...
                    if (message.Contains("StatusReq"))
                        Log.Debug(GetType().Name, "ReSendMessage", "Sending", "TX: {0}", message);
                    else
                        Log.Info(GetType().Name, "ReSendMessage", "Sending", "TX: {0}", message);
                }
            }
            catch (Exception ex)
            {
        ...
        public void SendMessage(int seqno, string message)
        {
            ...
            try
            {
                ...
                for (; OutGoingMessageQueue.ContainsKey(key); key = sequenceNumber)
                {
                    ...
                    Log.Debug(GetType().Name, nameof(SendMessage), "Sending",
                        "change message and increment sequence number from {0} to {1}", key, sequenceNumber);
                    message = message.Replace(string.Format("SeqNo=\"{0}\"", key),
                        string.Format("SeqNo=\"{0}\"", sequenceNumber));
                }

                if (message.Contains("StatusReq"))
                    Log.Debug(GetType().Name, nameof(SendMessage), "Sending", "TX: {0}", message);
                else
                    Log.Info(GetType().Name, nameof(SendMessage), "Sending", "TX: {0}", message);
        ...
        private void OnConnectionEvent(object sender, AuthoriseResponse e)
        {
            if (OutGoingMessageQueue.Count > 0)
            {
                ...
                OutGoingMessageQueue = new SortedList<int, string>(OutGoingMessageQueue
                    .Where(x =>
                        !x.Value.Contains("MsgID=\"102\" MsgName=\"StatusReq\""))
                    .Select(
                        message =>
                            new KeyValuePair<int, string>(message.Key - leastIndex + seqNo + 11,
                                message.Value.Replace(string.Format("SeqNo=\"{0}\"", message.Key),
                                    string.Format("SeqNo=\"{0}\"", message.Key - leastIndex + seqNo + 11))))
                    .ToDictionary(
                        newmessage => newmessage.Key,
                        newmessage => newmessage.Value));
                SendMessage(seqNo + 10,
                    SerializeToXML(new StatusRequest(seqNo + 10), StatusRequest.Serializer));
        ...
        private List<string> ProcessReceivedMessages(string xmlData) => xmlData.Split(
            new string[2]
            {
                "\r\n\r\n",
                "\n\n"
        ...
        private void Client_DataReceived(object sender, string data)
        {
            if (data.Contains("=\"StatusReport"))
                Log.Debug(GetType().Name, nameof(Client_DataReceived), "Receiving", "Rx: {0}", data);
            else
                Log.Info(GetType().Name, nameof(Client_DataReceived), "Receiving", "Rx: {0}", data);
            List<string> stringList = ProcessReceivedMessages(lastIncompleteMessage + data);
            lastIncompleteMessage = "";
            Console.WriteLine("Found {0} messages", stringList.Count);
            for (int index = 0; index < stringList.Count; ++index)
            {
                Console.WriteLine("[{0}] Processing message {1} of {2}", DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.ffff"), index + 1, stringList.Count);
                ...
                try
                {
                    Log.Info(GetType().Name, nameof(Client_DataReceived), "EventHandler",
                        "Message received. messageSeqNo={0} _controllerSequenceNumber={1}", num, _controllerSequenceNumber);
                    if (messageType == MessageType.ACK)
                    {
                        OutGoingMessageQueue.Remove(DeserializeFromXML<ACK>(str, ACK.Serializer)
                            .SequenceNumber);
                    }
                    else
                    {
                        ...
                        {
                            Log.Warning(GetType().Name, nameof(Client_DataReceived), "EventHandler",
                                "Old massage found messageSeqNo={0} _controllerSequenceNumber={1}", num, _controllerSequenceNumber);
                        }
                        else
                        {
                            ...
                            switch (messageType)
                            {
                                case MessageType.ReqErr:
                                    if (DeserializeFromXML<RequestError>(str, RequestError.Serializer).Body
                                ...
                                case MessageType.AuthoriseResp:
                                    AuthoriseResponse e =
                                        DeserializeFromXML<AuthoriseResponse>(str, AuthoriseResponse.Serializer);
        ...
        public void processResponse(string strData, MessageType messageType)
        {
            switch (messageType)
            {
                case MessageType.StatusReport:
                    OnStatusReportEvent(this,
                        DeserializeFromXML<StatusReport>(strData, StatusReport.Serializer));
                    break;
                case MessageType.DropStatus:
                    OnDropStatusResultEvent(this,
                        DeserializeFromXML<DropStatus>(strData, DropStatus.Serializer));
                    break;
                case MessageType.DropResult:
                    OnDropResultEvent(this,
                        DeserializeFromXML<DropResult>(strData, DropResult.Serializer));
                    break;
                case MessageType.BagRemovalReport:
                    BagRemovalReport bagRemovalReport =
                        DeserializeFromXML<BagRemovalReport>(strData, BagRemovalReport.Serializer);
                ...
                case MessageType.TransactionStatusResponse:
                    OnTransactionStatusEvent(this,
                        DeserializeFromXML<TransactionStatusResponse>(strData,
                            TransactionStatusResponse.Serializer));
                    break;
            }
        }
        ...
        internal void ShowDeviceController()
        {
            ...
            SendMessage(sequenceNumber,
                SerializeToXML(new ShowController(sequenceNumber), ShowController.Serializer));
        }

        public void Connect()
        {
            string xml = SerializeToXML(
                new AuthoriseRequest(_sequenceNumber, _clientID, _macAddress, _clientType),
        ...
        internal void ResetDevice() => SendMessage(_sequenceNumber,
            SerializeToXML(new CancelCurrentProcess(_sequenceNumber),
                CancelCurrentProcess.Serializer));

        internal void GetStatus()
        {
            int sequenceNumber = _sequenceNumber;
            SendMessage(sequenceNumber,
                SerializeToXML(new StatusRequest(sequenceNumber), StatusRequest.Serializer));
        }

        internal void ClearOutgoingMessageQueue() => OutGoingMessageQueue.Clear();

        internal void SetCurrency(string currency) => SendMessage(_sequenceNumber,
            SerializeToXML(new SelectCurrency(_sequenceNumber, currency),
                SelectCurrency.Serializer));
        ...
        internal void Count(string DropID, long transactionValue = 0)
        {
            ...
            SendMessage(sequenceNumber,
                SerializeToXML(
                    new DropRequest(sequenceNumber, Currency, UserID,
                        dropType: DropMode.ToCashAccSysString(), reference: TransactionID,
                        inputSubNumber: DropID, inputNumber: SessionID), DropRequest.Serializer));
        }
        ...
        internal void EndCount()
        {
            int sequenceNumber = _sequenceNumber;
            SendMessage(sequenceNumber,
                SerializeToXML(new DropEnd(sequenceNumber, "DONE"), DropEnd.Serializer));
        }
        ...
        internal void EscrowDrop()
        {
            int sequenceNumber = _sequenceNumber;
            SendMessage(sequenceNumber,
                SerializeToXML(new DropEnd(sequenceNumber, "ESCROW_DROP"), DropEnd.Serializer));
        }
        ...
        internal void EscrowReject()
        {
            int sequenceNumber = _sequenceNumber;
            SendMessage(sequenceNumber,
                SerializeToXML(new DropEnd(sequenceNumber, "ESCROW_REJECT"), DropEnd.Serializer));
        }
        ...
        internal void RequestPause()
        {
            int sequenceNumber = _sequenceNumber;
            SendMessage(sequenceNumber,
                SerializeToXML(new DropPause(sequenceNumber), DropPause.Serializer));
        }
        ...
        public void TransactionEnd()
        {
            int sequenceNumber = _sequenceNumber;
            SendMessage(sequenceNumber,
                SerializeToXML(new EndTransaction(sequenceNumber), EndTransaction.Serializer));
        }
        ...
        internal void EndCIT(string bagNumber)
        {
            int sequenceNumber = _sequenceNumber;
            SendMessage(sequenceNumber,
                SerializeToXML(new OpenBagRequest(sequenceNumber, bagNumber),
                    OpenBagRequest.Serializer));
        }
        ...
        internal void StartCIT(string sealNumber)
        {
            ...
            SendMessage(sequenceNumber,
                SerializeToXML(
                    new CloseBagRequest(sequenceNumber, sealNumber, CloseBagRequestAction.SUMMARY_REPORT),
                    CloseBagRequest.Serializer));
        }

        public void GetTransactionStatus()
        {
            IEnumerable<bool> source = OutGoingMessageQueue.Select(
                x =>
                {
                    string str = x.Value;
            ...
            int sequenceNumber = _sequenceNumber;
            SendMessage(sequenceNumber,
                SerializeToXML(new TransactionStatusRequest(sequenceNumber),
                    TransactionStatusRequest.Serializer));
        }
    }
}