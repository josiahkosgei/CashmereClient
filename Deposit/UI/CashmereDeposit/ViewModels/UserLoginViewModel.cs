...
namespace CashmereDeposit.ViewModels
{
    ...
    public class UserLoginViewModel : FormViewModelBase
    {
        ...
        private static IDeviceLoginRepository _deviceLoginRepository;
        private static IApplicationUserRepository _applicationUserRepository;
        ...
        public new async Task<int> FormValidation()
        {
            ...
            var dbApplicationUser =  _deviceRepository.GetByUserGroupAsync(device.UserGroup, Username);
            ...
            else
            {
                ...
                var deviceLogin1 =  _deviceLoginRepository.AddAsync(entity);
                ...
                else
                {
                    ...
                    {
                        var result = Task.Run(() => AuthenticationAsync(dbApplicationUser, Password)).Result;
                        if (!result.IsSuccess)
                        {
                            if (result.IsInvalidCredentials)
                            {
                                ...
                                if (dbApplicationUser.LoginAttempts >= ApplicationViewModel.DeviceConfiguration.LOGINFAIL_DEVICELOCK_RETRY_COUNT)
                                {
                                    ...
                                    dbApplicationUser.LoginAttempts = 0;
                                     _applicationUserRepository.UpdateAsync(dbApplicationUser);
                                    ApplicationViewModel.AlertManager.SendAlert(new AlertLoginFailed(device, DateTime.Now, FormErrorText, Username, dbApplicationUser));
                                    num1 = 3;
                                }
                                else
                        ...
                        else
                        {
                            ...
                            if (!dbApplicationUser.IsAdUser)
                            {
                                ...
                                {
                                    var passwordPolicy =  _passwordPolicyRepository.GetFirst();
                ...
                deviceLogin1.ChangePassword = new bool?(!dbApplicationUser.IsAdUser && dbApplicationUser.PasswordResetRequired);
                 _deviceLoginRepository.UpdateAsync(deviceLogin1);
            }
            // _depositorDBContext.SaveChangesAsync();
            return num1;
        }
