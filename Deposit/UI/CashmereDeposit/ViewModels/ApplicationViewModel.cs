    ...
    public class ApplicationViewModel : Conductor<Screen>, IShell
    {
        ...
        public CIT lastCIT
        {
            get
            {
                ...
                return _citRepository.GetByDeviceId(device.Id);
        ...
        private void SystemStartupChecks()
        {
            ...
            else if (_deviceRepository.GetDevice(Environment.MachineName) == null)
            {
        ...
        private void InitialiseDevice()
        {
            ...
            try
            {
                ...
                EscrowJam escrowJam = _escrowJamRepository.GetFirst();
        ...
        private void HandleIncompleteTransaction()
        {
            ...
            var transactions = _transactionRepository.GetCompleted();
        ...
        public async void HandleIncompleteSession()
        {
            ...
            var depositorSessions = _depositorSessionRepository.GetCompleted();
            foreach (DepositorSession depositorSession in depositorSessions)
            {
                ...
                var updated = _depositorSessionRepository.UpdateAsync(depositorSession);
            }
            ...
            try
            {
                ...
                if (ApplicationStatus.CashmereDeviceState == CashmereDeviceState.NONE || debugNoDevice)
                {
                    if (generateNewScreen)
                    {
                        ...
                        var guiScreenListScreen = _guiScreenListScreenRepository.GetByGUIScreenId(guiScreen.Id);
                        var currentGuiScreen = _gUIScreenRepository.GetByIdAsync(CurrentGUIScreen.Id);
        ...
        private async void DeviceManager_StatusReportEvent(object sender, DeviceStatusChangedEventArgs e)
        {
            ...
            if (device != null)
            {
                DeviceStatus deviceStatus = _deviceStatusRepository.GetByDeviceId(device.Id);
                if (deviceStatus == null)
                {
                    deviceStatus = _deviceStatusRepository.AddAsync(new DeviceStatus()
                    {
                        DeviceId = device.Id,
                ...
                else
                {
                    ...
                    var reslt = _deviceStatusRepository.UpdateAsync(deviceStatus);
                }
            }
        ...
        {
            var last_CIT = _citRepository.LastCIT(lastCIT.Id);
            var device = ApplicationModel.GetDeviceAsync();
            var ciTs = _citRepository.GetInCompleteByDeviceId(lastCIT.Id, device.Id);
            lock (BagOpenLock)
            {
                ...
                if (last_CIT != null && !last_CIT.Complete)
                {
                    ...
                    if (last_CIT != null)
                    {
                        ...
                        var reslt = _citRepository.UpdateAsync(last_CIT);
                        ...
                        foreach (var cit in ciTs)
                        {
                            ...
                            var output = _citRepository.UpdateAsync(cit);
                        }
                    }
               ...
               Transaction transaction)
        {
            ...
            try
            {
                ...
                var transactionTypeListItem = _transactionTypeListItemRepository.GetByIdAsync(transaction.TxType.Value);
        ...
        private async void ApplicationStatus_PropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            ...
            DeviceStatus deviceStatus = _deviceStatusRepository.GetByMachineName(Environment.MachineName);
            if (deviceStatus == null)
            {
        ...
        private async void DeviceManager_CITResultEvent(object sender, CITResult e)
        {
            ...
            var cit = _citRepository.GetByIdAsync(lastCIT.Id);
            PrintCITReceipt(cit);
        ...
        internal async void ShowFundsSourceScreen()
        {
            if ((bool)!CurrentTransaction?.TransactionLimits?.ShowFundsSource)
                return;
            var entity = _gUIScreenRepository.GetGUIScreenByCode(new Guid("33EC330E-FB51-4626-906D-1A3F77AAA5E2"));
            if (entity != null)
            {
        ...
        {
            this.ActivateItemAsync((object)new UserMessageScreenViewModel(title, this, DeviceConfiguration.USER_SCREEN_TIMEOUT, required)
            {
                Message = message
            });
        }
        ...
        internal async void LogoffUsers(bool forced = false)
        {
            try
            {
                if (CurrentUser != null)
                {
                    DeviceLogin deviceLogin = _deviceLoginRepository.GetFirst(CurrentUser.Id);
                    if (deviceLogin != null)
                    {
          ...
          string errorMessage)
        {
            ...
            ApplicationUser applicationUser = _applicationUserRepository.GetByIdAsync(user.Id);
            ...
            _userLockRepository.AddAsync(new UserLock()
            {
                Id = Guid.NewGuid(),
          ...
          bool isAuthorising = false)
        {
            try
            {
                return GetUserPermissionAsync(currentUser, activityString, isAuthorising) != null;
            }
            catch (Exception ex)
            {
                return false;
          ...
          bool isAuthenticating = false)
        {
            ...
            Activity Activity = _activityRepository.GetByName(activity);
            if (Activity == null)
                return null;
            return _permissionRepository.GetFirst(user, Activity.Id, isAuthenticating);
          ...
          Transaction transaction)
        {
            ...
            {
                Log.InfoFormat(applicationViewModel.GetType().Name, "Posting to live core banking", "Integation", "posting transaction {0}", transaction.ToString());
                TransactionTypeListItem transactionTypeListItem = _transactionTypeListItemRepository.GetByIdAsync(transaction.TxType.Value);
